...
Features
    ...
    System Enumeration: * Automatically extracts usernames from /etc/passwd.
...
Run the tool from your terminal by specifying the target URL, a known existing file (for baseline comparison), and optional session cookies.
Bash
...
-f / --file	A path to a known existing file on the server (used for baseline comparison).
-c / --cookie	(Optional) Cookie header for authenticated sessions.
How It Works

The tool functions by establishing a "normal state" response size based on the valid file provided. It then iterates through a predefined list of LFI payloads, comparing the response sizes and content patterns to detect successful file inclusion. If a vulnerability is confirmed, it proceeds to automate enumeration and exploitation tasks.
Contributing
